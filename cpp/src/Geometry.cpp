...
std::vector<SimplexVector> scaleBinarySet(const std::vector<BinaryVector> &X) {
  ...
  for (const auto &x : X) {
    result.push_back(scaleBinary(x));
  }
  return result;
}

std::vector<BinaryVector>
unscaleBinarySet(const std::vector<SimplexVector> &X) {
  std::vector<BinaryVector> result;
  result.reserve(X.size());
  for (const auto &x : X) {
    result.push_back(unscaleBinary(x));
  }
  return result;
}
