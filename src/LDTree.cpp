...
LDTree::LDTree(
    ...
    voronoi_(scaleBinarySet(points)),
    tree_(voronoi_)
{}
...
void LDTree::build(
    ...
    DynprogParams dynprogParams = DynprogParams{
        verbose,
        outputStream,
        logInterval,
        timeLimit - elapsedTime(startTime),
...
std::vector<BinaryVector> LDTree::query(
    const RealVector& cost,
    bool checkDomain
) const {
    if (checkDomain) {
        throw std::runtime_error("Domain checking not implemented yet.");
    }
...
void LDTree::generateNormalCode(std::ostringstream& out, int dim, const std::string& doc) const {
...
void LDTree::generateBenchmarkCode(std::ostringstream& out, int dim, const std::string& doc) const {
    out << "int main(int argc, char **argv) {\n";
    out << "    if (argc != 3) {\n";
    ...
    out << "                     (long double)(end.tv_nsec - start.tv_nsec) / 1e9;\n";
    out << "    printf(\"  runs       : %d\\n\", runs);\n";
    out << "    printf(\"  seed       : %d\\n\", seed);\n";
...
void LDTree::generateDotCode(
    std::ostringstream& out,
    const TernaryVector& split
) const {
    bool first = true;
