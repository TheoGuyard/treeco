...
Knapsack::Knapsack(Index numItems) 
    : numItems_(numItems) {
...
Knapsack::Knapsack(
    ...
    double capacity
) : numItems_(numItems), weights_(weights), capacity_(capacity) {
    dimension_ = numItems_;
...
Domain Knapsack::getCostDomain() const {
    Domain domain;
    for (Index i = 0; i < dimension_; ++i) {
        RealVector coeff(dimension_, 0.0);
        coeff[i] = 1.0;
        domain.push_back({coeff, 0.0, Relation::GT});
    }
    return domain;
}
