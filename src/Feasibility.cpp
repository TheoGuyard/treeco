...
Feasibility::Feasibility(
    const std::vector<TernaryVector>& pool,
    const Domain& fixedConstraints,
    double tolerance
) :
    pool_(pool),
...
void Feasibility::update(Index i, bool isNew) {
    ...
    bool anyInfeasible = std::find(
        relationsReduce_.begin(),
        relationsReduce_.end(),
        Relation::RF
    ) != relationsReduce_.end();
    if (anyInfeasible) {
        ...
        bool stillFeasible = (
            ...
            ((r == Relation::GT) && (value >= tolerance_)) ||
            (r == Relation::RT)
        ); 
        if (stillFeasible) {
