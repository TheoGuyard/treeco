...
protected:
    ...
    void SetUp() override {
        tempFilePath = "/tmp/treeco_test_io_" + std::to_string(std::rand()) + ".txt";
    }
...
TEST_F(IOTest, WriteDomainBasic) {
    Domain domain = {
        {RealVector{1.0, 2.0, -1.0}, 3.5, Relation::LT},
...
TEST_F(IOTest, WriteDomainAllRelations) {
    Domain domain = {
        {RealVector{1.0, 0.0}, 0.0, Relation::LT},
...
TEST_F(IOTest, WriteReadQueriesRoundTrip) {
    std::vector<RealVector> original = {
        {1.23456789, -9.87654321, 0.0},
...
TEST_F(IOTest, WriteReadQueriesPrecision) {
    ...
    std::vector<RealVector> original = {
        {1.234567890123456, -9.876543210987654}
    };
